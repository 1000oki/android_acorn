...
public class BlankFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        // 만일 fragment_blacnk.xml 문서에 myTextView라는 아이디를 가지고 있는 TextView의 참조값이 필요하다면.
        TextView a = binding.myTextView;

        return view;
    }

    // 프래그먼트가 레이아웃으로 가지고 있는 뷰가 파괴될 때 호출되는 메소드
    @Override
    public void onDestroyView() {
        super.onDestroyView();
        // FragmentBlankBinding 객체의 참조를 해제해서 메모리를 효율적으로 사용하도록 한다.
        binding = null;
        // 프래그먼트보다 뷰가 더 오래 살아 남기 때문에 프래그먼트는 사용되지 않는데
